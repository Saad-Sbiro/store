...
const SUGGESTIONS = ['Monitor stand', 'Mechanical keyboard', 'USB-C dock', 'Desk lamp', 'Noise-cancelling audio'];
...
export default function ExpandableSearchBar({ onSearch, className = '' }) {
  ...
  return (
    <Popover open={expanded} onOpenChange={setExpanded}>
      ...
      <PopoverContent align="end" className="w-[min(76vw,310px)]">
        <div className="grid gap-1">
          {SUGGESTIONS.map((item) => (
            <button
              ...
              className="rounded-[10px] px-3 py-2 text-left text-[13px] font-semibold text-white/82 transition-colors hover:bg-white/10 hover:text-white"
            >
              {item}
            </button>
          ))}
